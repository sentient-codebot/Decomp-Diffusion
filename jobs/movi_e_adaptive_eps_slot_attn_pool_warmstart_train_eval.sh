#!/bin/bash
# MOVi-E adaptive epsilon composition -- pooled slot-attention scalar weights.
#
...
#   sbatch jobs/movi_e_adaptive_eps_slot_attn_pool_warmstart_train_eval.sh
#SBATCH --job-name="movi-e-adapt-pool-50k"
...
RUN_DIR=results/movi-e_adaptive_eps_slot_attn_pool_warmstart
...
REPORT=docs/experiments/2026-05-28-movi-e-adaptive-eps-slot-attn-pool.md
MAX_STEPS=50000
RESOLUTION=256
PER_GPU_BATCH=8
...
if [ -z "$WARM_CKPT" ]; then
    echo "[movi-e-adapt-pool] FATAL: no warm-start checkpoint found under $SOURCE_RUN_DIR"
    exit 1
fi
if [ ! -d "$WARM_CKPT/dinoslotattentionencoder" ] || [ ! -d "$WARM_CKPT/unet2dconditionmodel" ]; then
    echo "[movi-e-adapt-pool] FATAL: checkpoint appears incomplete: $WARM_CKPT"
    exit 1
fi
echo "[movi-e-adapt-pool] warm-start checkpoint: $WARM_CKPT"
...
if [ ! -f "$TRAIN_SHARD_ROOT/samples.jsonl" ]; then
    echo "[movi-e-adapt-pool] FATAL: $TRAIN_SHARD_ROOT/samples.jsonl missing -- run jobs/movi_e_shard_wds.sh first."
    exit 1
fi
TRAIN_N=$(wc -l < "$TRAIN_SHARD_ROOT/samples.jsonl")
VAL_N=$(wc -l < "$VAL_SHARD_ROOT/samples.jsonl" 2>/dev/null || echo 0)
echo "[movi-e-adapt-pool] train frames=$TRAIN_N  val frames=$VAL_N"
...
uv run accelerate launch --multi_gpu --num_processes=2 --mixed_precision bf16 \
    ...
    --epsilon_composition slot_attn_pool \
    --resume_from_checkpoint latest \
    --max_train_steps "$MAX_STEPS"
TRAIN_RC=$?
TRAIN_END=$(date +%s)
echo "**************** [movi-e-adapt-pool] training finished (rc=$TRAIN_RC). ****************"
...
echo "[movi-e-adapt-pool] using checkpoint: ${CKPT:-<none found>}"
...
if [ -n "$CKPT" ]; then
    ...
    echo "[movi-e-adapt-pool] no checkpoint found -- skipping reconstruction eval."
    EVAL_RECON_RC=1
fi
...
# --- 3b. Eval: FG-ARI + mBO + mIoU on full validation split ------------------
...
END=$(date +%s)
echo "**************** [movi-e-adapt-pool] eval finished (recon rc=$EVAL_RECON_RC, metrics rc=$EVAL_METRICS_RC). ****************"
...
try:
    ...
    ax_loss.set_title("Adaptive pooled slot-attn epsilon weights -- MOVi-E")
...
if not os.path.exists(p):
    print("N/A N/A N/A N/A"); raise SystemExit
m = json.load(open(p))
...
if [ "$TRAIN_RC" -eq 0 ] && [ "$EVAL_RECON_RC" -eq 0 ] && [ "$EVAL_METRICS_RC" -eq 0 ]; then
    OVERALL="PASS -- 50k pooled-weight warm-start continuation completed"
else
...
**Slurm job:** ${SLURM_JOB_ID:-N/A} (script: jobs/movi_e_adaptive_eps_slot_attn_pool_warmstart_train_eval.sh, log: /home/nlin/prjs0993/Decomp-Diffusion/slurm_logs/slurm_${SLURM_JOB_ID}.log)
...
| wandb project | nan-team/$TRACKER_PROJECT_NAME |
...
| This pooled slot-attn adaptive run | $FG_ARI | $MBO | $MIOU |
...
echo "**************** [movi-e-adapt-pool] report written to $REPORT ($OVERALL) ****************"
