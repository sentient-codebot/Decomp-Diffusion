...
if [ ! -d "$TRAIN_IMG_ROOT" ]; then
    echo "[movi-e-dinov3-run] FATAL: $TRAIN_IMG_ROOT is missing -- run jobs/movi_e_preprocess.sh first."
    exit 1
fi
...
echo "[movi-e-dinov3-run] priming GPFS metadata..."
find "$TRAIN_IMG_ROOT" "$VAL_IMG_ROOT" -type f >/dev/null
echo "[movi-e-dinov3-run] done priming."
