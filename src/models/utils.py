...
class ColorMask:
    def __init__(
        ...
        self.log_image_resize = transforms.Resize(
            log_img_size,
        ...
        self.img_unnorm = transforms.Compose(
            [
                transforms.Normalize(
                    mean=[0.0, 0.0, 0.0], std=1 / torch.tensor(norm_std)
                ),
                transforms.Normalize(
                    mean=-torch.tensor(norm_mean), std=[1.0, 1.0, 1.0]
                ),
            ]
        )
    ...
    def get_heatmap(
        self, img, attn, recon=None, mask_pred_sorted=None, return_all=False
    ):
        """
        ...
        img = self.img_unnorm(img).clamp(0.0, 1.0)
        if recon is not None:
            recon = [self.img_unnorm(r).clamp(0.0, 1.0) for r in recon]
        ...
        if self.reshape_first and (
            attn.shape[-2] != img.shape[-2] or attn.shape[-1] != img.shape[-1]
        ):
            attn = transforms.Resize(
                size=img.shape[-2:],
                interpolation=transforms.InterpolationMode.BILINEAR,
                antialias=True,
            )(attn)
        ...
        if mask_pred_sorted is None:
            mask_pred = (
                attn.argmax(1, keepdim=True)
                == torch.arange(attn.size(1))[None, :, None, None]
            ).float()
        else:
        ...
        if mask_pred.shape[-2] != img.shape[-2] or mask_pred.shape[-1] != img.shape[-1]:
            mask_pred = transforms.Resize(
                size=img.shape[-2:], interpolation=transforms.InterpolationMode.NEAREST
            )(mask_pred)

        # b c h w
        img_overlay, color_mask = self._apply_mask(
            img, mask_pred, alpha=0.5, color=self.color
        )
        ...
        no_trans_heatmap, heatmap_on_image = self.apply_colormap_on_image(
            img_pil, attn, "gray"
        )

        heatmap_on_image = transforms.ToTensor()(heatmap_on_image.convert("RGB"))

        heatmap_on_image = rearrange(
            heatmap_on_image,
            "c (b s) (h w) -> b s c h w",
            b=bs,
            c=inp_channel,
        ...
        grid_image = torch.cat(
            [img[:, None], img_overlay[:, None], heatmap_on_image], dim=1
        )
        if recon is not None:
            if not isinstance(recon, list):
                recon = [recon]
            ...
            grid_image = torch.cat(
                [*[r[:, None] for r in recon if r is not None], grid_image], dim=1
            )
        grid_image = make_grid(
            rearrange(grid_image, "b n c h w -> (b n) c h w"),
            nrow=grid_image.size(1),
            padding=1,
            pad_value=0.8,
        )
        if return_all:
    ...
    def log_heatmap(self, img, attn, recon=None, mask_pred_sorted=None, path=None):
        assert path is not None or self.img_tmp_pth is not None, (
            "path is None and img_tmp_pth is None"
        )
