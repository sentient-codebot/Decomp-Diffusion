...
def hack_unet_for_pos(unet, pos_type="cartesian"):
    ...
    for block_name in blocks_to_add_pos:
        ...
        for block_idx, block in enumerate(blocks):
            if (
                isinstance(block, CrossAttnDownBlock2D)
                or isinstance(block, CrossAttnUpBlock2D)
                or isinstance(block, UNetMidBlock2DCrossAttn)
                or isinstance(block, UNetMidBlock2DSimpleCrossAttn)
            ):
                    ...
                    for transformer_block in attention_block.transformer_blocks:
                        ...
                        def forward_with_pos(self, *inputs, **kwargs):
                            ...
                            out = self.original_forward(
                                hidden_states, *inputs[1:], **kwargs
                            )
                            return out

                        bound_method = forward_with_pos.__get__(
                            transformer_block.attn1, transformer_block.attn1.__class__
                        )
                        transformer_block.attn1.forward = bound_method
                        ...
                        bound_method = forward_with_pos.__get__(
                            transformer_block.attn2, transformer_block.attn2.__class__
                        )
                        transformer_block.attn2.forward = bound_method
    return
